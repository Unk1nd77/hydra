...
esp_err_t SPI_Flash::erase_sector(size_t sector)
{
    ...
    if (result == ESP_OK) {
        ESP_LOGV(TAG, "erase_sector - sector=0x%08lx, result=0x%08x", (unsigned long)sector, result);
    } else {
        ESP_LOGE(TAG, "erase_sector - sector=0x%08lx, result=0x%08x", (unsigned long)sector, result);
    }
    return result;
}
...
esp_err_t SPI_Flash::write(size_t dest_addr, const void *src, size_t size)
{
    ...
    if (result == ESP_OK) {
        ESP_LOGV(TAG, "write - dest_addr=0x%08lx, size=0x%08lx, result=0x%08x", (unsigned long)dest_addr, (unsigned long)size, result);
    } else {
        ESP_LOGE(TAG, "write - dest_addr=0x%08lx, size=0x%08lx, result=0x%08x", (unsigned long)dest_addr, (unsigned long)size, result);
    }
    return result;
}
...
esp_err_t SPI_Flash::read(size_t src_addr, void *dest, size_t size)
{
    ...
    if (result == ESP_OK) {
        ESP_LOGV(TAG, "read - src_addr=0x%08lx, size=0x%08lx, result=0x%08x", (unsigned long)src_addr, (unsigned long)size, result);
    } else {
        ESP_LOGE(TAG, "read - src_addr=0x%08lx, size=0x%08lx, result=0x%08x", (unsigned long)src_addr, (unsigned long)size, result);
    }
    return result;
}
