...
{
    uint8_t data_byte = (data & 0x0F) | backlight_state | rs;
...
esp_err_t lcd_init(void)
{
    ...
    // Настройка дисплея
    lcd_write_byte(LCD_FUNCTION_SET | LCD_4BIT_MODE | LCD_2LINE | LCD_5x8DOTS, LCD_COMMAND);
    lcd_write_byte(LCD_DISPLAY_CONTROL | LCD_DISPLAY_ON | LCD_CURSOR_OFF | LCD_BLINK_OFF, LCD_COMMAND);
    lcd_write_byte(LCD_ENTRY_MODE_SET | LCD_ENTRY_LEFT, LCD_COMMAND);
    lcd_write_byte(LCD_CLEAR_DISPLAY, LCD_COMMAND);
